...
/**
 * Data-driven integrations grid. GoogleDataCard is a real, wired
 * connector shown first. Everything else in the catalog has no OAuth/API
 * integration built at all — their cards say "Coming soon" (disabled,
 * no onClick) instead of a "Connect" button that silently does nothing,
 * which was actively misleading about what the app could do.
 */
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
      ...
      {INTEGRATIONS_CATALOG.filter(i => i.platform_type !== 'ga4').map(item => (
        <div key={item.platform_type} className="hud-card p-4 flex flex-col items-start gap-3 opacity-60">
          <div className="hud-corner tl" /><div className="hud-corner br" />
          <div className="w-10 h-10 rounded-lg flex items-center justify-center bg-white/8">
            <Plug size={18} className="text-faint-c" />
          </div>
          <div className="flex-1">
          ...
          <button disabled className="text-xs px-3 py-1.5 rounded-lg font-medium text-faint-c bg-white/8 w-full cursor-not-allowed">
            Coming soon
          </button>
        </div>
      ))}
